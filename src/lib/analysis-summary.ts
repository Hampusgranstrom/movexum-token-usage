...
import { STATUS_CONFIG, type AnalysisSummary, type LeadStatus } from "@/lib/types";
...
type VitalMetric = "CLS" | "INP" | "LCP" | "FCP" | "TTFB";
...
function rateMetric(metric: VitalMetric, value: number): "good" | "needs-improvement" | "poor" {
  if (metric === "CLS") {
...
export async function getAnalysisSummary(days: number): Promise<AnalysisSummary> {
  const supabase = getSupabaseAdmin();
  if (!supabase) {
    throw new Error("Supabase ej konfigurerad.");
  }

  const safeDays = Math.min(180, Math.max(7, Math.trunc(days) || 30));
  const sinceDate = new Date(Date.now() - safeDays * 86400000).toISOString();
  const prevSinceDate = new Date(Date.now() - safeDays * 2 * 86400000).toISOString();
  ...
  const [
    ...
    supabase.from("modules").select("id,slug,name"),
    supabase
  ...
  const leadsPerDay = countPerDay(leads, safeDays);
  const messagesPerDay = countPerDay(messages, safeDays);
  ...
  const vitalsByMetric = new Map<VitalMetric, Array<{ value: number; rating: "good" | "needs-improvement" | "poor"; path: string }>>();
  const vitalsPrevByMetric = new Map<VitalMetric, Array<{ value: number; rating: "good" | "needs-improvement" | "poor"; path: string }>>();
  const vitalsByPath = new Map<string, Array<{ metric: VitalMetric; value: number; rating: "good" | "needs-improvement" | "poor" }>>();
  for (const e of events) {
    const isCurrentWindow = e.created_at >= sinceDate;
    if (e.event_type === "web_vital") {
      const metric = e.metadata?.metric;
      const path = typeof e.metadata?.path === "string" ? e.metadata.path : "/";
      const value = Number(e.metadata?.value);
      if (
        (metric === "CLS" || metric === "INP" || metric === "LCP" || metric === "FCP" || metric === "TTFB") &&
        Number.isFinite(value)
      ) {
        const rating = e.metadata?.rating ?? rateMetric(metric, value);
        if (isCurrentWindow) {
          const existingMetric = vitalsByMetric.get(metric) ?? [];
          existingMetric.push({ value, rating, path });
          vitalsByMetric.set(metric, existingMetric);

          const pathKey = `${path}::${metric}`;
          const existingPath = vitalsByPath.get(pathKey) ?? [];
          existingPath.push({ metric, value, rating });
          vitalsByPath.set(pathKey, existingPath);
        } else {
          const existingPrevMetric = vitalsPrevByMetric.get(metric) ?? [];
          existingPrevMetric.push({ value, rating, path });
          vitalsPrevByMetric.set(metric, existingPrevMetric);
        }
      }
      continue;
    }
  ...
  const webVitals: AnalysisSummary["performance"]["webVitals"] = (["CLS", "INP", "LCP", "FCP", "TTFB"] as VitalMetric[])
    .map((metric) => {
      const rows = vitalsByMetric.get(metric) ?? [];
      const prevRows = vitalsPrevByMetric.get(metric) ?? [];
      const values = rows.map((row) => row.value);
      const prevValues = prevRows.map((row) => row.value);
      const poorCount = rows.filter((row) => row.rating === "poor").length;
      const p75 = percentile(values, 0.75);
      const prevP75 = percentile(prevValues, 0.75);
      return {
        metric,
        samples: rows.length,
        avg: rows.length > 0 ? values.reduce((sum, value) => sum + value, 0) / rows.length : 0,
        p75,
        prevP75,
        p75Delta: prevP75 > 0 ? (p75 - prevP75) / prevP75 : 0,
        poorRate: rows.length > 0 ? poorCount / rows.length : 0,
        rating: rateMetric(metric, p75),
      };
    })
    .filter((row) => row.samples > 0);

  const slowPaths: AnalysisSummary["performance"]["slowPaths"] = Array.from(vitalsByPath.entries())
    .map(([key, rows]) => {
      const [path, metric] = key.split("::") as [string, VitalMetric];
      const values = rows.map((row) => row.value);
      const poorCount = rows.filter((row) => row.rating === "poor").length;
      return {
        path,
  ...
  for (const lead of leads) {
    const municipality = lead.municipality?.trim();
